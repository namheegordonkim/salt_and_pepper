...
from learners import RewardFunction, ContextualLearner, ContextualRewardFunction
...
class AppState:
    def __init__(self, learner: ContextualLearner, reward_vis: Vis, policy_vis: Vis):
    ...
    def gui():
        ...
        if clicked or app_state.first_time:
            ...
            if app_state.point_cloud_actor is not None:
                ...
                app_state.point_cloud = None
                app_state.point_cloud_actor = None

        changed, app_state.context_idx = imgui.slider_int(
            "Context Index", app_state.context_idx, 0, 3
        )
        if changed:
            if app_state.point_cloud_actor is not None:
                pl.remove_actor(app_state.point_cloud_actor)
        ...
        changed, app_state.n_iters = imgui.slider_int(
            "Number of Gradient Iterations", app_state.n_iters, 1, 1000
        )
        changed, app_state.lr = imgui.slider_float(
            "Learning Rate", app_state.lr, 0, 0.1, format="%.5f"
        )
        ...
        if clicked or (app_state.stepping and app_state.step_stage == 3):
            print("Learning")
            app_state.learner.learn(app_state.n_iters)

        # if clicked or app_state.first_time:
        with torch.no_grad():
            policy_mu = (
                app_state.learner.mu.forward(
                    torch.as_tensor([[app_state.context_idx]], dtype=torch.float)
                )
                .cpu()
                .detach()
                .numpy()[0]
            ...
            policy_sigma = (
                torch.exp(
                    app_state.learner.log_sigma.forward(
                        torch.as_tensor([[app_state.context_idx]], dtype=torch.float)
                    )
                )
                .cpu()
        ...
        # imgui.text(f"Current mu: {policy_mu}")
        # imgui.text(f"Current sigma: {policy_sigma}")
        ...
        pdf_ry = app_state.learner.reward_function.evaluate(
            app_state.context_idx,
            app_state.reward_vis.mesh.points[:, 0],
            app_state.reward_vis.mesh.points[:, 1],
        )
        app_state.reward_vis.mesh.point_data["reward"] = pdf_ry
        app_state.reward_vis.mesh.points[:, 2] = pdf_ry

        if app_state.learner.samples is not None:
            samples = np.zeros((app_state.learner.samples.shape[0], 3))
            samples[:, :2] = app_state.learner.samples
            samples[:, -1] = 1
            context_yes = app_state.learner.context_idxs == app_state.context_idx
            samples_for_context = samples[context_yes]

            if app_state.point_cloud_actor is None and context_yes.sum() > 0:
                app_state.point_cloud = pv.PolyData(samples_for_context)
                app_state.point_cloud.point_data["weight"] = 0
                app_state.point_cloud_actor = pl.add_pointcloud(
                    app_state.point_cloud, scalars="weight", color="blue", point_size=10
                )

            app_state.point_cloud.points = samples_for_context
            if app_state.point_cloud_actor is not None:
                if app_state.learner.rewards is not None:
                    app_state.point_cloud.points[:, -1] = app_state.learner.rewards[
                        context_yes
                    ]
                if app_state.learner.weights is not None:
                    app_state.point_cloud.point_data["weight"] = (
                        app_state.learner.weights[context_yes]
                    )
...
def main(args):
    ...
    true_mus = np.array([[0.5, 0.5], [-0.5, -0.5], [0.5, -0.5], [-0.5, 0.5]])
    true_sigmas = np.array([0.5, 0.5, 0.5, 0.5])
    reward_function = ContextualRewardFunction(true_mus, true_sigmas)
    learner = ContextualLearner(reward_function)
